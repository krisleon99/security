...
//if ($cat != 'todas') {
//	$query = "SELECT * FROM $tipos[$cat], ST_X(ST_Transform($tipos[$cat].geom,4326)) as lng, ST_Y(ST_Transform($tipos[$cat].geom,4326)) as lat order by fecha asc";
...
$query = "select count(id) as num from $table group by mes order by num desc limit 1";
